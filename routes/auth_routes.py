...
def submit_login():
    ...
    if password == config.ADMIN_PASSWORD:                                 # השוואת הסיסמה שהוזנה לסיסמה הנכונה
        ...
        # אם יש תורים שמועדם כבר עבר ועדיין לא הוכרע מה קרה איתם - מקפיצים מיד את מסך הסקירה,
        # כדי שהמנהל/ת יסמן/תסמן אילו בוצעו ואילו מבוטלים, במקום שהם יישארו תלויים באוויר.
        import models.appointments as appointments                            # ייבוא מקומי, למניעת תלות מעגלית
        if appointments.list_past_pending_appointments():                       # אם יש תורים שממתינים להחלטה
            return redirect(url_for("admin.review_past_appointments"))            # מעבר ישיר למסך הסקירה

        return redirect(next_page)                                            # אחרת - מעבר למסך שביקשו במקור
