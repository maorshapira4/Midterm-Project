...
ALL_INTENTS = (
    ...
    INTENT_OPENING_HOURS, INTENT_GREETING, INTENT_ADMIN_REQUEST, INTENT_OTHER,
)
...
_SYSTEM_INSTRUCTION_TEMPLATE = (
    ...
    "'admin_request' (בקשה לפעולה של מנהל/ת הקליניקה - למשל לראות רשימות של לקוחות או של כל "
    "התורים במערכת, לשנות סטטוס של תור, להנפיק חשבונית, לטפל בלידים, לראות הכנסות, למחוק "
    "לקוח, או לבקש מידע על אדם אחר), "
    "'other' (כל דבר אחר). "
    "\n"
    ...
    "\n"
    "requested_time - שעה שהלקוח/ה מבקש/ת, בפורמט 'HH:MM' (24 שעות), או null. "
    "\n"
...
def extract(free_text_message, available_service_names=None):
    ...
    if gender not in ("גבר", "אישה"):                                  # מקבלים רק שני הערכים החוקיים בסכמה
        gender = None
